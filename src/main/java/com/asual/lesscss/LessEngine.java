...
import com.asual.lesscss.loader.AbsoluteResourceLoader;
...
public class LessEngine {
	...
	public LessEngine(String baseDir) {
		this(new LessOptions(),baseDir);
	}
	
	public LessEngine(LessOptions options,String baseDir) {
		this(options, defaultResourceLoader(options,baseDir));
	}
	...
	private static ResourceLoader defaultResourceLoader(LessOptions options,String baseDir) {
		ResourceLoader resourceLoader = new ChainedResourceLoader(
				...
				new HTTPResourceLoader(),new AbsoluteResourceLoader(baseDir));
		if(options.isCss()) {
	...
	public String compile(File input, boolean compress) throws LessException {
		try {
			long time = System.currentTimeMillis();
			String location = "file://"+input.getAbsolutePath();
